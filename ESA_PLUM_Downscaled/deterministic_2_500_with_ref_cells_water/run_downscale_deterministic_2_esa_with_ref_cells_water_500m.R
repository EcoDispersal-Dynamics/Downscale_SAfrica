...
# ================================================================
# ESA_PLUM_Downscaled: deterministic_2 runner at 500m (assign_ref_cells = TRUE, water-filled reference)
# - Uses ESA WorldCover 2021 500m water-filled raster for complete coverage
# - Iteratively updates the reference each year
# - Scenarios limited to SSP1_RCP26 and SSP5_RCP85
# - Regions: 4 x 2 grid (8 regions total)
# Created: 2025-10-15 (derived from run_downscale_deterministic_2_esa_with_ref_cells.R)
# ================================================================
...
if (length(args) < 2) {
  ...
  cat("Usage: Rscript run_downscale_deterministic_2_esa_with_ref_cells_water_500m.R <scenario_name> <region_id>\n")
  quit(status = 1)
}
...
if (!(scenario_name %in% allowed_scenarios)) {
  cat("ERROR: Only SSP1_RCP26 and SSP5_RCP85 are supported.\n")
  quit(status = 1)
}
...
module_root <- file.path(esa_root, "deterministic_2_500_with_ref_cells_water")
log_dir <- file.path(module_root, "logs")
dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)

timestamp <- format(Sys.time(), "%Y%m%d_%H%M%S")
log_file <- file.path(log_dir, paste0(scenario_name, "_region", region_id, "_det2_500_with_ref_cells_water_ESA_", timestamp, ".log"))
...
log_line(paste("Deterministic_2_500_with_ref_cells_water ESA run for", scenario_name, "region", region_id))
...
if (file.exists(esa_waterfilled_path)) {
  ...
  esa_ref_path <- file.path(base_dir, "ESA_WorldCover/processed/processed_500m/ESA_WorldCover_2021_aligned_500m.tif")
}
...
initial_ref <- file.path(output_dir, paste0(scenario_name, "_region", region_id, "_initial_ref_map_water_500m.tif"))
terra::writeRaster(region_ref, initial_ref, overwrite = TRUE)
log_line(paste("Saved initial ref:", initial_ref))

existing_outputs <- list.files(output_dir, pattern = paste0(scenario_name, "_region", region_id, "_.*_500m_Discrete_Time1\\.tif$"), full.names = TRUE)
start_year_index <- 1
...
if (length(existing_outputs) > 0) {
  ...
  if (length(plum_files_check) > 0) {
    for (i in seq_along(plum_files_check)) {
      pf <- plum_files_check[i]
      ym <- regexpr("_([0-9]{4}_[0-9]{4})_", pf, perl = TRUE)
      if (ym > 0) {
        ystr <- substr(pf, ym + 1, ym + 9)
        candidate <- file.path(output_dir, paste0(scenario_name, "_region", region_id, "_", ystr, "_500m_Discrete_Time1.tif"))
...
log_line(paste("Found", length(plum_files), "PLUM files"))

if (length(existing_outputs) > 0 && start_year_index > 1) {
...
build_synergy <- function(plum_layer_names, modis_classes) {
  ...
  synergy <- matrix(0, nrow = length(plum_layer_names), ncol = length(modis_classes),
                    dimnames = list(plum_layer_names, modis_classes))
  for (cat in names(allocations)) {
    valid <- intersect(names(allocations[[cat]]), modis_classes)
    if (length(valid) > 0) {
      synergy[cat, valid] <- allocations[[cat]][valid]
    } else {
      synergy[cat, ] <- 1 / length(modis_classes)
    }
  }
...
for (year_index in start_year_index:length(plum_files)) {
  ...
  if (length(modis_values) == 0) {
    ...
    empty_path <- file.path(output_dir, paste0(scenario_name, "_region", region_id, "_", year_str, "_500m_Discrete_Time1.tif"))
    terra::writeRaster(region_ref, empty_path, overwrite = TRUE)
    current_ref <- empty_path
    next
  }
  modis_classes <- paste0("LC", modis_values)
  log_line(paste("Region ESA classes:", paste(sort(modis_classes), collapse = ", ")))

  if (is.null(match_LC_classes)) {
    plum_names <- names(region_plum)
    match_LC_classes <- build_synergy(plum_names, modis_classes)
...
log_line("Deterministic_2_500_with_ref_cells_water ESA run completed.")
