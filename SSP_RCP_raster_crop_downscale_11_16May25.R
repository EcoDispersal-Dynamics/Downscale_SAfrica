...
results <- foreach(country_name = countries, .packages = c("terra", "data.table", "LandScaleR")) %dopar% {
  tryCatch({
    ...
    for (i in seq_along(plum_files)) {
      ...
      # Look for newest Time1.tif file and rename it
      generated_files <- list.files(downscale_output_dir, pattern = paste0(country_name, ".*", years, ".*Time1\\.tif$"), full.names = TRUE)
      ...
      file.rename(generated_files[1], renamed_file)
